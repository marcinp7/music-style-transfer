...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \coordinate (split) at (4, 1);

        \node[tensor] (melody) at (6, 4) {melody};
        \node[tensor] (rhythm) at (6, 2) {rhythm};
        \node[tensor] (style) at (6, 0) {style};

        \coordinate (merge2) at (8, 1);
        \coordinate (split2) at (9, 1);
        \coordinate (merge3) at (10, 3);

        \node[tensor] (output) at (12, 3) {output\\song content};
        \node[tensor] (output_info) at (12, 1) {output\\song information};
        ...
        \draw[-Latex] (merge3) to [out=0,in=180] (output);
    \end{tikzpicture}
    \caption{Data flow of the model (dashed part is optional).}
    \label{fig:data_flow}
\end{figure}
